  ...
  return profileData && (
    <div>
      ...
      <div className='flex flex-col gap-4 m-5'>
        ...
        <div>
          <img className='bg-indigo-500/80 w-full sm:max-w-64 rounded-lg' src={profileData.image} alt="" />
        </div>
        ...
        <div className='flex-1 border border-stone-100 rounded-lg px-8 py-7 bg-white '>
          ...
          <div className='flex gap-1 pt-2'>
            ...
            {/* <input onChange={() => setProfileData(prev => ({ ...prev, available: !prev.available }))}  checked={profileData.available} type="checkbox" name="" id="" disabled={!isEdit} /> */}
            <label htmlFor="">Available</label>
          </div>

          {
            isEdit
              ? <button onClick={updateProfile} className='px-4 py-1 border border-indigo-500 text-sm rounded-full mt-5 hover:bg-indigo-500 hover:text-white transition-all  '>Save</button>
