...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    if (!docInfo) {
      return; // Agar docInfo nahi hai to function se bahar nikal jao
    }
    setDocSlots([])
    ...
    for (let i = 0; i < 7; i++) {
      ...
      //setting end time of the date with date
      ...
      while (currentDate < endTime) {
        ...
        //if any slot is already booked or not
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      let month = date.getMonth() + 1//if not added 1 than 0 is for jan
      ...
      if (data.success) {
        ...
        getDoctorsData()
        navigate('/my-appointments') //to snd user on my appointment page
      } else {
  ...
  return docInfo && (
    <div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700 '>
        ...
        <button onClick={bookAppointment} className='rounded-full bg-indigo-500 text-sm  text-white font-light px-14 py-3 my-6 cursor-pointer'>Book an appointment </button>
      </div>
